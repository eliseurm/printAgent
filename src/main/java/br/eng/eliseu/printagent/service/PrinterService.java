...
public class PrinterService {
    ...
    public PrinterService(PrintAgentProperties properties) {
    ...
    public ImpressoraResponseDTO consultar(String nome) {
        PrintService p = encontrar(nome);
        PrintService d = PrintServiceLookup.lookupDefaultPrintService();
        return new ImpressoraResponseDTO(p.getName(), d != null && d.getName().equals(p.getName()), status(p));
    }
    ...
    public CapacidadesImpressoraResponseDTO capacidades(String nome) {
        ...
        for (Class<?> categoria : p.getSupportedAttributeCategories()) {
            Object valor = p.getDefaultAttributeValue((Class<? extends Attribute>) categoria);
            dados.put(categoria.getSimpleName(), valor == null ? List.of() : List.of(valor.toString()));
        }
    ...
    public void imprimir(TrabalhoImpressao t) throws Exception {
        ...
        if ("application/pdf".equals(t.getTipoConteudo())) {
            imprimirPdfLinux(t, bytes);
            return;
        }
    ...
    private byte[] conteudo(TrabalhoImpressao t) {
        return switch (t.getTipoConteudo()) {
            ...
            case "application/pdf" -> {
                try {
                    yield Base64.getDecoder().decode(t.getConteudo());
                } catch (IllegalArgumentException e) {
                    throw new ApiException(HttpStatus.BAD_REQUEST, "CONTEUDO_INVALIDO", "PDF Base64 inválido.", "documento.conteudo");
                }
            }
    ...
    private void imprimirPdfLinux(TrabalhoImpressao t, byte[] bytes) throws Exception {
        if (!System.getProperty("os.name").toLowerCase(Locale.ROOT).contains("linux"))
            throw new ApiException(HttpStatus.UNSUPPORTED_MEDIA_TYPE, "TIPO_CONTEUDO_NAO_SUPORTADO_PELO_PROVIDER", "PDF não é suportado pelo Provider Windows na versão 1.0.0.", "documento.tipoConteudo");
        Path arquivo = Files.createTempFile("print-agent-", ".pdf");
        try {
            Files.write(arquivo, bytes, StandardOpenOption.TRUNCATE_EXISTING);
            List<String> cmd = new ArrayList<>(List.of("lp", "-d", t.getImpressora()));
            t.getConfiguracoes().forEach((k, v) -> {
                cmd.add("-o");
                cmd.add(k + "=" + v);
            });
            cmd.add(arquivo.toString());
            Process p = new ProcessBuilder(cmd).redirectErrorStream(true).start();
            if (!p.waitFor(properties.getProvider().getTimeoutComandoSegundos(), TimeUnit.SECONDS)) {
                p.destroyForcibly();
                throw new IOException("Timeout ao executar lp.");
            }
    ...
    private PrintService encontrar(String nome) {
        return Arrays.stream(PrintServiceLookup.lookupPrintServices(null, null)).filter(p -> p.getName().equals(nome)).findFirst().orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "IMPRESSORA_NAO_ENCONTRADA", "A impressora '" + nome + "' não está instalada.", "impressora.nome"));
    }

    private String status(PrintService p) {
        PrinterIsAcceptingJobs a = p.getAttribute(PrinterIsAcceptingJobs.class);
        return PrinterIsAcceptingJobs.NOT_ACCEPTING_JOBS.equals(a) ? "STOPPED" : "READY";
    }
}
